...
- `packages/content/src/pipeline/run.ts` — pass `authored.units` into `assignUnits`.
...
- Tests: `packages/content/test/authored.test.ts`, `packages/content/test/units.test.ts`, `packages/content/test/run.test.ts` (fixture update).
...
## Task 3: Capture L1, regenerate, and prove byte-identical output
